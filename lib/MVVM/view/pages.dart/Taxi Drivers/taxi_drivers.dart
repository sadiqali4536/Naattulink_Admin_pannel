...
class _TaxiDriversPageState extends State<TaxiDriversPage> {
  ...
  String _searchQuery = '';
  Set<String> _selectedTaxiIds = {};
  final ScrollController _verticalScrollController = ScrollController();
  final ScrollController _horizontalScrollController = ScrollController();
  late Stream<QuerySnapshot> _taxiStream;
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
            'Are you sure you want to $action ${_selectedTaxiIds.length} selected taxi driver(s)?',
          ),
          actions: [
            TextButton(
  ...
  Future<void> _executeBulkAction(
    ...
    final ids = _selectedTaxiIds.toList();
    int successCount = 0;
    ...
    setState(() {
      _selectedTaxiIds.clear();
    });

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Successfully processed $successCount taxi driver(s)'),
        ),
      );
    }
  }
  ...
  Widget _buildBulkActionToolbar(List<QueryDocumentSnapshot> filteredDocs) {
    if (_selectedTaxiIds.isEmpty) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(
            '${_selectedTaxiIds.length} selected',
            style: TextStyle(
                    ...
                    docs.where((d) {
                      ...
                      if (_searchQuery.isNotEmpty) {
                        ...
                        final reg =
                            ...
                            '';
                        final mainStand =
                            data['main_stand']?.toString().toLowerCase() ?? '';

                        if (!name.contains(searchLower) &&
                            !phone.contains(searchNoSpaces) &&
                            !reg.contains(searchNoSpaces) &&
                            !mainStand.contains(searchLower)) {
                          searchMatches = false;
                        }
                      }
              ...
              label: Row(
                children: [
                  Checkbox(
                    value:
                        docs.isNotEmpty &&
                        _selectedTaxiIds.length == docs.length,
                    onChanged: (val) {
                      setState(() {
                        if (val == true) {
                          _selectedTaxiIds.addAll(docs.map((d) => d.id));
                        } else {
                          _selectedTaxiIds.clear();
                        }
                      });
                  ...
                  cells: [
                    DataCell(
                      Row(
                        ...
                        children: [
                          Checkbox(
                            value: _selectedTaxiIds.contains(doc.id),
                            onChanged: (val) {
                              setState(() {
                                if (val == true) {
                                  _selectedTaxiIds.add(doc.id);
                                } else {
                                  _selectedTaxiIds.remove(doc.id);
                                }
                              });
                            },
                            shape: RoundedRectangleBorder(
