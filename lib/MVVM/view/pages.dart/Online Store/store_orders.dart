...
class _StoreOrdersPageState extends State<StoreOrdersPage> {
  ...
  static const _orderStatuses = [
    'All Status', 'Pending', 'Confirmed', 'Processing',
  ...
  bool _can(String action) => _session.hasPermission(Modules.storeOrders, action);
  ...
  Future<void> _fetchOrders() async {
    ...
    try {
      ...
      _calculateStats(snapshot.docs.map((d) => StoreOrderModel.fromFirestore(d)).toList());
  ...
  void _calculateStats(List<StoreOrderModel> all) {
    ...
    for (final o in all) {
      ...
      if (o.orderStatus == 'Processing' || o.orderStatus == 'Confirmed') processing++;
  ...
  Future<void> _updateOrderStatus(StoreOrderModel order, String newStatus) async {
    try {
  ...
  void _showOrderDetails(StoreOrderModel order) {
    showDialog(context: context, builder: (ctx) => _OrderDetailsDialog(order: order));
  }
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () { Navigator.pop(ctx); _updateOrderStatus(order, selected); },
      ...
      child: Row(
        ...
        children: [
          Column(
            ...
            children: [
              Row(children: [
                Text('Dashboard', style: GoogleFonts.inter(fontSize: 12, color: const Color(0xFF64748B))),
                const Padding(
                  padding: EdgeInsets.symmetric(horizontal: 8),
                  child: Icon(Icons.chevron_right_rounded, size: 16, color: Color(0xFF94A3B8)),
                ),
                Text('Online Store', style: GoogleFonts.inter(fontSize: 12, color: const Color(0xFF64748B))),
                const Padding(
                  padding: EdgeInsets.symmetric(horizontal: 8),
                  child: Icon(Icons.chevron_right_rounded, size: 16, color: Color(0xFF94A3B8)),
                ),
                Text('Store Orders', style: GoogleFonts.inter(
              ...
              const SizedBox(height: 4),
              Text('Manage and track all customer orders from the online store.',
  ...
  Widget _buildStatsRow() {
    final revenue = _statRevenue >= 1000
        ? '${(_statRevenue / 1000).toStringAsFixed(1)}K'
        : _statRevenue.toStringAsFixed(0);
    return Row(children: [
      _statCard('Total Orders', _statTotal.toString(), Icons.receipt_long_outlined, const Color(0xFF3B82F6)),
      const SizedBox(width: 16),
      _statCard('Pending', _statPending.toString(), Icons.hourglass_empty_rounded, const Color(0xFFF59E0B)),
      const SizedBox(width: 16),
      _statCard('Processing', _statProcessing.toString(), Icons.autorenew_rounded, const Color(0xFF8B5CF6)),
      const SizedBox(width: 16),
      _statCard('Delivered', _statDelivered.toString(), Icons.check_circle_outline_rounded, const Color(0xFF10B981)),
      const SizedBox(width: 16),
      _statCard('Revenue', '₹$revenue', Icons.currency_rupee_rounded, const Color(0xFF0EA5E9)),
    ]);
  }
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w500, color: const Color(0xFF64748B))),
              const SizedBox(height: 4),
              Text(value, style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF1E293B))),
            ]),
          ),
        ]),
      ),
  ...
  Widget _buildToolbar() {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        _dropdown(value: _selectedStatus, items: _orderStatuses, icon: Icons.local_shipping_outlined,
            onChanged: (v) { setState(() => _selectedStatus = v!); _fetchOrders(); }),
        const SizedBox(width: 12),
        _dropdown(value: _selectedPayment, items: _paymentStatuses, icon: Icons.payment_rounded,
            onChanged: (v) { setState(() => _selectedPayment = v!); _fetchOrders(); }),
      ]),
    );
  }

  Widget _dropdown({required String value, required List<String> items, required IconData icon, required ValueChanged<String?> onChanged}) {
    return Container(
      height: 42,
          ...
          Scrollbar(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                rows: _orders.map((order) => DataRow(cells: [
                  ...
                  DataCell(_badge(order.paymentStatus, _paymentColor(order.paymentStatus), _paymentBg(order.paymentStatus))),
                  DataCell(_badge(order.orderStatus, _statusColor(order.orderStatus), _statusBg(order.orderStatus))),
                  DataCell(_dateCell(order.createdAt)),
                  if (_can(Perms.edit)) DataCell(_actionsCell(order)),
                ])).toList(),
              ),
            ),
          ),
  ...
  Widget _orderCell(StoreOrderModel o) => Column(
    ...
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('#${o.orderNumber}', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFF3B82F6))),
      Text(o.id.length > 10 ? '${o.id.substring(0, 10)}...' : o.id,
  ...
  Widget _customerCell(StoreOrderModel o) => SizedBox(
    ...
    child: Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(o.customerName, overflow: TextOverflow.ellipsis,
            style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1E293B))),
        if (o.customerPhone.isNotEmpty)
          Text(o.customerPhone, style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF64748B))),
      ],
    ),
  );

  Widget _itemsCell(StoreOrderModel o) => Row(mainAxisSize: MainAxisSize.min, children: [
    Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(color: const Color(0xFFF1F5F9), borderRadius: BorderRadius.circular(6)),
      child: Text('${o.items.length} item${o.items.length == 1 ? '' : 's'}',
          style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: const Color(0xFF475569))),
    ),
    if (o.itemCount > 0) ...[
      const SizedBox(width: 4),
      Text('x${o.itemCount}', style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF94A3B8))),
    ],
  ]);

  Widget _badge(String text, Color color, Color bg) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
    decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(20), border: Border.all(color: color.withOpacity(0.25))),
    child: Text(text, style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w600, color: color)),
  );

  Widget _dateCell(DateTime? date) {
    if (date == null) return Text('—', style: GoogleFonts.inter(color: const Color(0xFF94A3B8)));
    final diff = DateTime.now().difference(date);
    final rel = diff.inMinutes < 60 ? '${diff.inMinutes}m ago'
    ...
    return Column(
      ...
      children: [
        Text('${date.day}/${date.month}/${date.year}',
            style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w500, color: const Color(0xFF1E293B))),
        Text(rel, style: GoogleFonts.inter(fontSize: 11, color: const Color(0xFF94A3B8))),
      ],
    );
  }

  Widget _actionsCell(StoreOrderModel order) => Row(mainAxisSize: MainAxisSize.min, children: [
    IconButton(icon: const Icon(Icons.visibility_outlined, size: 18), color: const Color(0xFF3B82F6),
        tooltip: 'View Details', splashRadius: 20, onPressed: () => _showOrderDetails(order)),
    IconButton(icon: const Icon(Icons.edit_note_rounded, size: 20), color: const Color(0xFF8B5CF6),
        tooltip: 'Update Status', splashRadius: 20, onPressed: () => _showStatusUpdateDialog(order)),
  ]);
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 24),
        Text('No Orders Found', style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.bold, color: const Color(0xFF1E293B))),
        const SizedBox(height: 8),
        Text(
          _searchQuery.isNotEmpty || _selectedStatus != 'All Status' || _selectedPayment != 'All Payments'
  ...
  Color _statusBg(String s) {
    switch (s) {
      ...
      case 'Processing': return const Color(0xFFF5F3FF);
...
class _OrderDetailsDialog extends StatelessWidget {
            ...
            child: Row(
              ...
              children: [
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text('#${order.orderNumber}', style: GoogleFonts.inter(fontSize: 13, color: const Color(0xFF3B82F6), fontWeight: FontWeight.w600)),
                ]),
                IconButton(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  _chip(order.paymentStatus, _pColor(order.paymentStatus)),
                ]),
                const SizedBox(height: 20),
                _section('Customer Information'),
                const SizedBox(height: 12),
                _row(Icons.person_outline_rounded, 'Name', order.customerName),
                ...
                if (order.deliveryAddress.isNotEmpty)
                  _row(Icons.location_on_outlined, 'Address', order.deliveryAddress),
                const SizedBox(height: 20),
                _section('Order Information'),
                const SizedBox(height: 12),
                _row(Icons.payment_rounded, 'Payment', order.paymentMethod),
                _row(Icons.calendar_today_outlined, 'Ordered On',
                ...
                Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('Total Amount', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: const Color(0xFF475569))),
  ...
  Widget _section(String text) => Text(text,
      style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: const Color(0xFF475569), letterSpacing: 0.3));
  ...
  Widget _row(IconData icon, String label, String value) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(width: 110, child: Text(label, style: GoogleFonts.inter(fontSize: 13, color: const Color(0xFF64748B)))),
      Expanded(child: Text(value, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w500, color: const Color(0xFF1E293B)))),
    ]),
  );
    ...
    return Container(
      ...
      child: Row(children: [
          ...
          child: ClipRRect(
            ...
            child: imageUrl != null && imageUrl.isNotEmpty
                ? Image.network(imageUrl, fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => const Icon(Icons.inventory_2_outlined, size: 20, color: Color(0xFF94A3B8)))
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Qty: $qty', style: GoogleFonts.inter(fontSize: 12, color: const Color(0xFF64748B))),
        ])),
